/* ============================================================================
 * js/app.js (COMPLETO)
 * Splash VALE GAMES + PWA + Lista de Compras
 * ========================================================================== */
...
document.addEventListener("DOMContentLoaded", () => {
  ...
  // Tempo mínimo de splash (efeito AAA)
  const splashMinMs = 950;
  ...
  function loadSavedLists() {
    ...
    lists.forEach((list) => {
      ...
      info.innerHTML = `<div><strong>${list.name}</strong><br><small>${new Date(list.createdAt).toLocaleDateString()}</small></div><div><strong>${money(list.total)}</strong></div>`;
  ...
  function exportListToPdf(items, listName = "Lista de Compras") {
    ...
    if (jsPdfLoaded && window.jspdf.autoTable) {
      ...
      doc.text(`Total: ${money(total)}`, 14, (doc.lastAutoTable?.finalY || 30) + 10);
  ...
  async function init() {
    if (window.DlcLoader && window.DlcLoader.init) {
      try { await window.DlcLoader.init(); } catch (_) {}
    }
